<template>
  <div class="checkout-page">
    ...
    <div class="container">
      <div class="checkout-addr">
        <div class="page-title-normal">
          <h2 class="page-title-h2"><span>核算</span></h2>
        </div>
        <!-- process step -->
          ...
          <div class="addr-list">
            <ul>
              <li
                ...
                @click="
                  checkIndex = index;
                  slectedAddressId = item.addressId;
                "
              >
                <dl>
                  <dt>{{ item.userName }}</dt>
                ...
                <div class="addr-opration addr-del">
                  <a
                    href="javascript:;"
                    class="addr-del-btn"
                    @click="delAddress(item.addressId)"
                  >
            ...
            <ul>
              <li class="check">
                ...
                <div class="shipping-tips">
                  <p>
                    Once shipped，Order should arrive in the destination in 1-7
                    business days
                  </p>
                </div>
              </li>
        ...
        <div class="next-btn-wrap">
          <router-link
            ...
            :to="{
              path: 'orderConfirm',
              query: { addressId: slectedAddressId },
            }"
            >Next</router-link
          >
        </div>
    ...
    <Modal v-bind:mdshow="isMdShow" @close="closeModal">
      ...
      <div slot="btnGroup">
        <a href="javascript:;" class="btn btn--m" @click="delAddressConfirm"
          >确认</a
        >
        <a href="javascript:;" class="btn btn--m" @click="isMdShow = false"
          >关闭</a
        >
      </div>
    </Modal>
        ...
        <div class="md-content">
          <div class="confirm-tips">
            <div class="error-wrap">
              <span class="error error-show"  v-show="postTip">邮编格式错误（6位）</span>
                <span class="error error-show"  v-show="numerror">电话格式错误</span>
                 <span class="error error-show"  v-show="adderror">地址不能为纯数字或字母，写详细地址</span> 
            </div>
            <ul>
          ...
          <div class="login-wrap">
            <a href="javascript:;" class="btn-login" @click="addAddress"
              >添加新地址</a
            >
          </div>
        </div>
...
export default {
    ...
    methods:{
      ...
      setDefault(addressId){
        axios.post('/users/setDefult',{addressId:addressId}).then((response)=>{
          let res = response.data;
          if (res.status=='0') {
            console.log("set defult suc");
            this.init()
          }
        })
      },
      ...
      addAddress(){
          ...
          if(this.streetName!=''&&this.tel!=''&& this.postCode!=''){
             var reg = /^[0-9][0-9]{5}$/;
          var reg1 = /^[1][3,4,5,6,7,8,9][0-9]{9}$/;
          var num=/^\d+$/;
          var a=/^[a-zA-Z]+$/;
          var p=reg.test(this.postCode);
          var t=reg1.test(this.tel);
          var addnum=num.test(this.streetName);
          var adda=a.test(this.streetName)
          if (!p) {
            this.postTip=true;
          }
          if (!t) {
